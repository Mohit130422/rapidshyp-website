jQuery(function($) {
    $(".checkLiveDemo").on("click", function() {
        gtag4("event", "Click_Demo", {
            event_category: "Demo",
        })
    });
    $(".topHeader a.link-click").on("click", function() {
        gtag4("event", "Top_Strip_Click", {
            event_category: "Header_Strip",
        })
    });
    $(".cargoxCta").on("click", function() {
        gtag4("event", "CargoX_Click", {
            event_category: "ShiprocketX",
        })
    });
    $(".shipxCta").on("click", function() {
        gtag4("event", "ShipX_Click", {
            event_category: "ShiprocketX",
        })
    });
    $(".openApp").on("click", function() {
        gtag4("event", "Open_in_app", {
            event_category: "Shiprocket",
        })
    });
    $(".loginCta").on("click", function() {
        gtag4("event", "Login_Click", {
            event_category: "Shiprocket",
        })
    });
    $(".registerCta").on("click", function() {
        gtag4("event", "Register_Click", {
            event_category: "Shiprocket",
        })
    });
    $(".button-light-blue, .whiteBtn, .blueBtn, .linkBtn").addClass("link-click");
    ...
    var parsedUtm = getParsedCookie(getCookie("UTM"));
    if (!getCookie("UTM")) {
    ...
    var paramsKeys = ["utm_source", "utm_campaign", "utm_content", "utm_medium", "utm_term", ];
    for (var i = 0; i < paramsKeys.length; i++) {
        var key = paramsKeys[i];
        if (searchParams.get(key)) {
            UTM[key] = searchParams.get(key)
        } else if (getCookie("UTM")) {
            var parsedUTMData = parsedUtm;
    ...
    function getParams() {
        var params = "";
        if ("utm_source" in window.location.search) {
            params = window.location.search + "&pathname=" + getCookie("pathname") + "&referrer=" + getCookie("referrer")
        } else {
            params = "?pathname=" + getCookie("pathname") + "&referrer=" + getCookie("referrer") + "&utm_source=" + getCookie("utm_source") + "&utm_campaign=" + getCookie("utm_campaign") + "&utm_content=" + getCookie("utm_content") + "&utm_term=" + getCookie("utm_term")
        }
        return params
    }
    $(".link-click").click(function() {
        ...
        webengage.track("Clicked on Link", {
            cta_text: button_text,
            destination_url: href,
        })
    });
    $(".Get_Ref_code").click(function() {
        var href = $(this).attr("href");
        var button_text = "Get_Ref_code";
        webengage.track("Get_Ref_code", {
            cta_text: button_text,
            destination_url: href,
        })
    });
...
var existingHistory = getCookie("browse_history");
var historyArray = existingHistory ? existingHistory.split(",") : [];
if (historyArray.length === 0 || historyArray[historyArray.length - 1] !== currentURL) {
    historyArray.push(currentURL)
}
if (historyArray.length > 10) {
    historyArray = historyArray.slice(-10)
}
var updatedHistory = historyArray.join(",");
var expiryDate = new Date();
expiryDate.setDate(expiryDate.getDate() + 30);
var domain = window.location.hostname.replace(/^www\./, "");
document.cookie = "browse_history=" + updatedHistory + "; expires=" + expiryDate.toUTCString() + "; domain=" + domain + "; path=/;";
